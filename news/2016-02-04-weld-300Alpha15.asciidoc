...
Previously, the default executor of the underlying `CompletableFuture` (`ForkJoinPool#commonPool()` by default) was used.
This is important in Java EE where `ForkJoinPool#commonPool()` is not a good fit.
...
In this way, you don't need to create an interceptor class for simple use cases:
[source,java]
----
class MyExtension {
